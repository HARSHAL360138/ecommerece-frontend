  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // Fetch user-specific orders (existing logic)
        const orderData = await fetchWithAuth(
          `https://ecommerce-backend-y1bv.onrender.com/api/buynow/user/${userData._id}`
        );

        // Fetch all orders (newly added endpoint)
        const allOrdersResponse = await fetchWithAuth(
          "https://ecommerce-backend-y1bv.onrender.com/api/orders"
        );

        const allOrders = allOrdersResponse?.orders || [];

        // Merge both order lists (avoid duplicates by _id)
        const combinedOrders = [
          ...(orderData || []),
          ...allOrders.filter(
            (newOrder) =>
              !(orderData || []).some((existing) => existing._id === newOrder._id)
          ),
        ];
    ...
    <div className="min-h-screen bg-[#f9f9f9] py-10 px-4 text-[#002349]">
      ...
      {loading ? (
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          {orders.map((order, index) => {
            const product =
              order.product || order.products?.[0]?.product || {};
            const shipping = product.shipping || {};
            const warranty = product.warranty || {};
            const share = product.share || {};
            const dimensions = shipping.dimensions || {};
